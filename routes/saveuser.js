...
router.post('/:userId/themes/:themeId', async (req, res, next) => {
    ...
    try {
        ...
        if(settings_data?.current && Object.keys(settings_data?.current).length > 1 || settings_data?.current?.sections && Object.keys(settings_data?.current?.sections).length > 0){
            let saveSettingsSchema = await fs.writeFile(path.join(__dirname, `../users/user-${userId}/theme-${themeId}/config/settings_data.json`), JSON.stringify(settings, null, 2), 'utf-8');
            console.log(saveSettingsSchema, "Save Settings Schema");
        }
        
        if(Object.keys(templates).length > 0){
            Object.entries(templates).forEach(async ([key, val]) => {
                const sectionsFile = await fs.readFile(path.join(__dirname, `../users/user-${userId}/theme-${themeId}/templates/${key}.json`), 'utf-8');
                ...
                const saveSectionsSchema = await fs.writeFile(path.join(__dirname, `../users/user-${userId}/theme-${themeId}/templates/${key}.json`), JSON.stringify(parseSections, null, 2), 'utf-8'); 
        ...
        .src(req.protocol + '://' + req.get('host') + '/view/users/' + userId + '/themes/' + themeId + '?page=index&share=zjo6KVWbwz', ['1440x900'], {crop: true, filename: 'screen-' + themeId})
        .src(req.protocol + '://' + req.get('host') + '/view/users/' + userId + '/themes/' + themeId + '?page=index&share=zjo6KVWbwz', ['414x736'], {crop: true, filename: 'm-screen-' + themeId})
